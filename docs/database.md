...
The schema is migration-driven and organized into five main layers:
...
These tables are created by `008_billing.sql` and are present in the schema, but are unused in the OSS build. Billing is not enabled by default and no billing logic runs in this branch.
