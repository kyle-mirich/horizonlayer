...
npx -y horizonlayer@2.0.0 backup /secure/path/horizonlayer-data.hlbackup
```
...
```bash
npx -y horizonlayer@2.0.0 recover /secure/path/horizonlayer-data.hlbackup
```
...
```bash
npx -y horizonlayer@2.0.0 recover /secure/path/horizonlayer-data.hlbackup --yes
```
...
- **Failure after canonical and Qdrant validation:** valid recovered data is retained. HorizonLayer does not overwrite it merely because the final service restart failed; run `npx -y horizonlayer@2.0.0 doctor`, then `npx -y horizonlayer@2.0.0 setup`.
