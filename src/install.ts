...
// Windows cannot spawn `npx` directly: the executable is an `npx.cmd` shim that
// Node-based clients fail to resolve. Launching through `cmd /c` is the portable form.
export function adaptMcpServersForPlatform(config: unknown, platform: NodeJS.Platform): unknown {
  if (platform !== 'win32' || !isRecord(config) || !isRecord(config.mcpServers)) {
    return config;
  }
  const servers = config.mcpServers;
  const adaptedServers: Record<string, unknown> = {};
  let changed = false;
  for (const [name, server] of Object.entries(servers)) {
    let adapted = server;
    if (isRecord(server) && server.command === 'npx') {
      adapted = {
        ...server,
        command: 'cmd',
        args: ['/c', 'npx', ...(Array.isArray(server.args) ? server.args : [])],
      };
      changed = true;
    }
    adaptedServers[name] = adapted;
  }
  return changed ? { ...config, mcpServers: adaptedServers } : config;
}

async function adaptStagedPluginMcp(pluginPath: string, platform: NodeJS.Platform): Promise<void> {
  ...
  try {
    ...
    throw new Error(`Bundled HorizonLayer MCP configuration at ${mcpPath} is unreadable: ${message}`);
  }
  const adapted = adaptMcpServersForPlatform(config, platform);
  if (adapted !== config) {
    await writeFile(mcpPath, `${JSON.stringify(adapted, null, 2)}\n`, 'utf8');
  }
}
...
async function installClaude(
  ...
  const staged = await stageClaudeMarketplace(marketplaceSource, target, skills, platform);
  try {
...
async function installCodex(
  ...
  const staged = await stageCodexPlugin(source, target, skills, platform);
  try {
