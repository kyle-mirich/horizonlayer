...
function buildManagedDbConfig(): ManagedDbConfig {
  return {
    containerName: process.env.HORIZONDB_DOCKER_CONTAINER_NAME ?? 'horizondb-postgres',
    database: process.env.HORIZONDB_DB_NAME ?? 'horizon_layer',
    host: process.env.HORIZONDB_DB_HOST ?? '127.0.0.1',
    image: process.env.HORIZONDB_DOCKER_IMAGE ?? 'pgvector/pgvector:pg17',
    password: process.env.HORIZONDB_DB_PASSWORD ?? 'postgres',
    port: parseNumber(process.env.HORIZONDB_DB_PORT, 5432),
    user: process.env.HORIZONDB_DB_USER ?? 'postgres',
    volumeName: process.env.HORIZONDB_DOCKER_VOLUME_NAME ?? 'horizondb-postgres-data',
  };
}
...
async function main(): Promise<void> {
  process.env.SERVER_TRANSPORT = process.env.SERVER_TRANSPORT || 'stdio';
