...
describe('agent-first core MCP contract', () => {
  ...
  it('supports the canonical workspace, session, page, search, and run flow', async () => {
    ...
    await expect(call('page', {
      action: 'get', page_id: 'p_AAAAAAAAAAAAAAAAAAAACg',
    })).resolves.toMatchObject({ ok: true });
    expect(pageMocks.getPage).toHaveBeenCalledWith(ids.page, expect.anything());
    await expect(call('search', {
      mode: 'records',
      query: 'native search',
      scope: { kind: 'workspace', workspace_id: ids.workspace },
      ...
      result: {
        mode: 'records',
        format: 'compact',
        records: [{ ref: 'p_AAAAAAAAAAAAAAAAAAAACg', rev: 1, updated: '2026-01-02T00:00:00Z' }],
        truncated: false,
      },
    });
    await expect(call('search', {
      ...
      ok: true,
      result: { mode: 'records', format: 'full', records: [{ id: ids.page }] },
    });
  ...
  it('routes rag mode explicitly and preserves dependency failures', async () => {
    await expect(call('search', {
      ...
      ok: true,
      meta: { limit: 8 },
      result: { mode: 'rag', format: 'compact', chunks: [{ rank: 1 }], truncated: false },
    });
  ...
  it('reports already archived or restored entities as non-retryable conflicts', async () => {
    const cases: Array<[
      toolName: string,
      input: Record<string, unknown>,
      mutation: { mockRejectedValueOnce: (error: Error) => unknown },
      message: string,
    ]> = [
      ['workspace', { action: 'archive', workspace_id: ids.workspace, revision: 1 }, workspaceMocks.archiveWorkspace, `Workspace ${ids.workspace} is already archived`],
      ['workspace', { action: 'restore', workspace_id: ids.workspace, revision: 1 }, workspaceMocks.restoreWorkspace, `Workspace ${ids.workspace} is already restored`],
      ['page', { action: 'archive', page_id: ids.page, revision: 1 }, pageMocks.archivePage, `Page ${ids.page} is already archived`],
      ['page', { action: 'restore', page_id: ids.page, revision: 1 }, pageMocks.restorePage, `Page ${ids.page} is already restored`],
      ['page', { action: 'block_archive', block_id: ids.block, revision: 1 }, pageMocks.archivePageBlock, `Block ${ids.block} is already archived`],
      ['page', { action: 'block_restore', block_id: ids.block, revision: 1 }, pageMocks.restorePageBlock, `Block ${ids.block} is already restored`],
      ['database', { action: 'archive', database_id: ids.database, revision: 1 }, databaseMocks.archiveDatabase, `Database ${ids.database} is already archived`],
      ['database', { action: 'restore', database_id: ids.database, revision: 1 }, databaseMocks.restoreDatabase, `Database ${ids.database} is already restored`],
      ['database', { action: 'property_archive', property_id: ids.property, revision: 1 }, databaseMocks.archiveDatabaseProperty, `Database property ${ids.property} is already archived`],
      ['database', { action: 'property_restore', property_id: ids.property, revision: 1 }, databaseMocks.restoreDatabaseProperty, `Database property ${ids.property} is already restored`],
      ['row', { action: 'archive', row_id: ids.row, revision: 1 }, rowMocks.archiveRow, `Row ${ids.row} is already archived`],
      ['row', { action: 'restore', row_id: ids.row, revision: 1 }, rowMocks.restoreRow, `Row ${ids.row} is already restored`],
      ['link', { action: 'archive', link_id: ids.link, revision: 1 }, linkMocks.archiveLink, `Link ${ids.link} is already archived`],
      ['link', { action: 'restore', link_id: ids.link, revision: 1 }, linkMocks.restoreLink, `Link ${ids.link} is already restored`],
    ];

    for (const [toolName, input, mutation, message] of cases) {
      mutation.mockRejectedValueOnce(new Error(message));
      await expect(call(toolName, input)).resolves.toMatchObject({
        ok: false,
  ...
  it('routes every lifecycle action to its query with complete bounded options', async () => {
    const calls: Array<[string, Record<string, unknown>]> = [
      ['workspace', { action: 'get', workspace_id: ids.workspace, include_archived: true }],
      ['workspace', { action: 'update', workspace_id: ids.workspace, revision: 1, icon: '📚' }],
      ['workspace', { action: 'archive', workspace_id: ids.workspace, revision: 1 }],
      ['workspace', { action: 'restore', workspace_id: ids.workspace, revision: 1 }],
      ['session', { action: 'list', workspace_id: ids.workspace, status: ['active'], limit: 2, offset: 1 }],
      ['session', { action: 'resume', session_id: ids.session, workspace_id: ids.workspace, max_items: 2 }],
      ['session', { action: 'close', session_id: ids.session }],
      ['page', {
        action: 'get', page_id: ids.page, session_id: ids.session, include_archived: true,
        block_limit: 2, block_offset: 1,
      }],
      ['page', {
        action: 'list', workspace_id: ids.workspace, session_id: ids.session, parent_page_id: ids.page,
        tags: ['design'], min_importance: 0.2, include_archived: true, limit: 2, offset: 1,
      }],
      ['page', { action: 'update', page_id: ids.page, revision: 1, tags: ['design'] }],
      ['page', {
        action: 'append', page_id: ids.page, revision: 1, session_id: ids.session,
        blocks: [{ block_type: 'heading', content: 'Details', metadata: { level: 1 } }],
      }],
      ['page', { action: 'block_update', block_id: ids.block, revision: 1, metadata: { reviewed: true } }],
      ['page', { action: 'archive', page_id: ids.page, revision: 1 }],
      ['page', { action: 'restore', page_id: ids.page, revision: 1 }],
      ['page', { action: 'block_archive', block_id: ids.block, revision: 1 }],
      ['page', { action: 'block_restore', block_id: ids.block, revision: 1 }],
      ['database', {
        action: 'list', workspace_id: ids.workspace, tags: ['design'], include_archived: true, limit: 2, offset: 1,
      }],
      ['database', { action: 'get', database_id: ids.database, include_archived: true }],
      ['database', { action: 'update', database_id: ids.database, revision: 1, tags: ['design'] }],
      ['database', { action: 'archive', database_id: ids.database, revision: 1 }],
      ['database', { action: 'restore', database_id: ids.database, revision: 1 }],
      ['database', {
        action: 'property_add', database_id: ids.database, revision: 1,
        property: { name: 'Status', property_type: 'select', options: { choices: ['Open'] } },
      }],
      ['database', {
        action: 'property_update', property_id: ids.property, revision: 1,
        options: { choices: ['Open', 'Closed'] },
      }],
      ['database', { action: 'property_archive', property_id: ids.property, revision: 1 }],
      ['database', { action: 'property_restore', property_id: ids.property, revision: 1 }],
      ['row', { action: 'get', row_id: ids.row, include_archived: true }],
      ['row', {
        action: 'query', database_id: ids.database, filters: [{ property: 'Name', operator: 'is_empty' }],
        tags: ['design'], include_archived: true, limit: 2, offset: 1,
      }],
      ['row', { action: 'update', row_id: ids.row, revision: 1, values: { Name: 'Updated' } }],
      ['row', { action: 'archive', row_id: ids.row, revision: 1 }],
      ['row', { action: 'restore', row_id: ids.row, revision: 1 }],
      ['link', {
        action: 'list', workspace_id: ids.workspace, link_type: 'references', include_archived: true,
        limit: 2, offset: 1,
      }],
      ['link', { action: 'archive', link_id: ids.link, revision: 1 }],
      ['link', { action: 'restore', link_id: ids.link, revision: 1 }],
      ['run', { action: 'get', run_id: ids.run, checkpoint_limit: 2, checkpoint_offset: 1 }],
      ['run', {
        action: 'list', workspace_id: ids.workspace, session_id: ids.session, agent_name: 'codex',
        status: ['running'], limit: 2, offset: 1,
      }],
      ['run', { action: 'checkpoint', run_id: ids.run, metadata: { phase: 'review' } }],
      ['run', { action: 'finish', run_id: ids.run, outcome: 'cancelled', result: { reason: 'paused' } }],
    ];

    for (const [toolName, input] of calls) {
      await expect(call(toolName, input)).resolves.toMatchObject({ ok: true });
    }

    expect(sessionMocks.listSessions).toHaveBeenCalledWith(expect.objectContaining({ limit: 3, offset: 1 }));
    expect(pageMocks.getPage).toHaveBeenCalledWith(ids.page, {
      session_id: ids.session,
      include_archived: true,
      block_limit: 2,
      block_offset: 1,
    });
    expect(pageMocks.appendPageBlocks).toHaveBeenCalledWith(ids.page, [{
      block_type: 'heading', content: 'Details', metadata: { level: 1 },
    }], { revision: 1, session_id: ids.session });
    expect(databaseMocks.addDatabaseProperty).toHaveBeenCalledWith(ids.database, {
      database_revision: 1,
      name: 'Status',
    ...
    expect(linkMocks.listLinks).toHaveBeenCalledWith(expect.objectContaining({ limit: 3, offset: 1 }));
    expect(runMocks.getRun).toHaveBeenCalledWith(ids.run, { checkpoint_limit: 2, checkpoint_offset: 1 });
  });

  it('returns not-found envelopes for every nullable query result', async () => {
    const cases: Array<[
      toolName: string,
      input: Record<string, unknown>,
      query: { mockResolvedValueOnce: (value: null) => unknown },
    ]> = [
      ['workspace', { action: 'get', workspace_id: ids.workspace }, workspaceMocks.getWorkspace],
      ['workspace', { action: 'update', workspace_id: ids.workspace, revision: 1, name: 'Updated' }, workspaceMocks.updateWorkspace],
      ['workspace', { action: 'archive', workspace_id: ids.workspace, revision: 1 }, workspaceMocks.archiveWorkspace],
      ['workspace', { action: 'restore', workspace_id: ids.workspace, revision: 1 }, workspaceMocks.restoreWorkspace],
      ['session', { action: 'resume', session_id: ids.session }, sessionMocks.resumeSession],
      ['session', { action: 'close', session_id: ids.session }, sessionMocks.closeSession],
      ['page', { action: 'get', page_id: ids.page }, pageMocks.getPage],
      ['page', { action: 'update', page_id: ids.page, revision: 1, title: 'Updated' }, pageMocks.updatePage],
      ['page', { action: 'block_update', block_id: ids.block, revision: 1, content: 'Updated' }, pageMocks.updatePageBlock],
      ['page', { action: 'archive', page_id: ids.page, revision: 1 }, pageMocks.archivePage],
      ['page', { action: 'restore', page_id: ids.page, revision: 1 }, pageMocks.restorePage],
      ['page', { action: 'block_archive', block_id: ids.block, revision: 1 }, pageMocks.archivePageBlock],
      ['page', { action: 'block_restore', block_id: ids.block, revision: 1 }, pageMocks.restorePageBlock],
      ['database', { action: 'get', database_id: ids.database }, databaseMocks.getDatabase],
      ['database', { action: 'update', database_id: ids.database, revision: 1, name: 'Updated' }, databaseMocks.updateDatabase],
      ['database', { action: 'archive', database_id: ids.database, revision: 1 }, databaseMocks.archiveDatabase],
      ['database', { action: 'restore', database_id: ids.database, revision: 1 }, databaseMocks.restoreDatabase],
      ['database', { action: 'property_update', property_id: ids.property, revision: 1, name: 'Updated' }, databaseMocks.updateDatabaseProperty],
      ['database', { action: 'property_archive', property_id: ids.property, revision: 1 }, databaseMocks.archiveDatabaseProperty],
      ['database', { action: 'property_restore', property_id: ids.property, revision: 1 }, databaseMocks.restoreDatabaseProperty],
      ['row', { action: 'get', row_id: ids.row }, rowMocks.getRow],
      ['row', { action: 'update', row_id: ids.row, revision: 1, tags: ['updated'] }, rowMocks.updateRow],
      ['row', { action: 'archive', row_id: ids.row, revision: 1 }, rowMocks.archiveRow],
      ['row', { action: 'restore', row_id: ids.row, revision: 1 }, rowMocks.restoreRow],
      ['link', { action: 'archive', link_id: ids.link, revision: 1 }, linkMocks.archiveLink],
      ['link', { action: 'restore', link_id: ids.link, revision: 1 }, linkMocks.restoreLink],
      ['run', { action: 'get', run_id: ids.run }, runMocks.getRun],
      ['run', { action: 'checkpoint', run_id: ids.run, summary: 'progress' }, runMocks.checkpointRun],
      ['run', { action: 'finish', run_id: ids.run, outcome: 'completed' }, runMocks.finishRun],
    ];

    for (const [toolName, input, query] of cases) {
  ...
  it('contains query errors within every core tool envelope', async () => {
    const consoleErrorSpy = vi.spyOn(console, 'error').mockImplementation(() => undefined);
    const cases: Array<[
      toolName: string,
      input: Record<string, unknown>,
      query: { mockRejectedValueOnce: (error: Error) => unknown },
    ]> = [
      ['workspace', { action: 'get', workspace_id: ids.workspace }, workspaceMocks.getWorkspace],
      ['session', { action: 'close', session_id: ids.session }, sessionMocks.closeSession],
      ['page', { action: 'get', page_id: ids.page }, pageMocks.getPage],
      ['database', { action: 'get', database_id: ids.database }, databaseMocks.getDatabase],
      ['row', { action: 'get', row_id: ids.row }, rowMocks.getRow],
      ['link', { action: 'list', workspace_id: ids.workspace }, linkMocks.listLinks],
      ['run', { action: 'get', run_id: ids.run }, runMocks.getRun],
    ];

    for (const [toolName, input, query] of cases) {
      query.mockRejectedValueOnce(new Error(`${toolName} unavailable`));
      await expect(call(toolName, input)).resolves.toMatchObject({
        ok: false,
    ...
    searchMocks.resolveSearchScope.mockRejectedValueOnce(new Error('scope unavailable'));
    await expect(call('search', {
      mode: 'records', query: 'design', scope: { kind: 'workspace', workspace_id: ids.workspace },
    })).resolves.toMatchObject({
      ok: false,
      error: { code: 'INTERNAL', retryable: false },
    });
