...
const DEPENDENCY_UNAVAILABLE_CODE_PREFIXES = [
  '08', // connection_exception class
  '53', // insufficient_resources class
] as const;

const DEPENDENCY_UNAVAILABLE_RUNTIME_CODES = new Set([
  'EAI_AGAIN',
  'ECONNABORTED',
  'ECONNREFUSED',
  'ECONNRESET',
...
export function errorEnvelopeFromUnknown(action: string, error: unknown) {
  ...
  if (classified.code === 'INTERNAL') {
    console.error(`[${action}] Internal tool error: ${classified.message}`);
  }
  return errorEnvelope(
    action,
    classified.code === 'INTERNAL'
      ? 'HorizonLayer could not complete the request'
      : classified.message,
    classified.code,
