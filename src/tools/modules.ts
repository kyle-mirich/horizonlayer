...
import { expandReference } from '../references.js';
...
// Accept both canonical UUIDs and the compact references (p_xxx, r_xxx, ...)
// returned by search and traversal responses, matching the knowledge tools.
const Id = z.string().trim().min(1).transform((value, ctx) => {
  try {
    return expandReference(value);
  } catch {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: 'Expected a HorizonLayer UUID or compact reference',
    });
    return z.NEVER;
  }
});
