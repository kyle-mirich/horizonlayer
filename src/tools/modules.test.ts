...
describe('compact module handlers', () => {
  ...
  it('accepts compact references wherever identifiers are accepted', async () => {
    ...
    const compactDependency = compactReference('link', id2);
    await issues.execute({ action: 'project.get', input: { project_id: compactProject } });
    expect(mocks.getIssueProject).toHaveBeenCalledWith(id, undefined);
    await issues.execute({ action: 'dependency.archive', input: {
      dependency_id: compactDependency, revision: 1,
    } });
    expect(mocks.archiveIssueDependency).toHaveBeenCalledWith(id2, 1);
    const invalid = await issues.execute({ action: 'project.get', input: { project_id: 'p_not-a-reference' } });
