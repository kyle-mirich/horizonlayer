...
async function assertWorkspaceArchiveTransition(
  id: string,
  revision: number,
  archived: boolean
): Promise<void> {
  ...
  const { rows } = await pool.query<{ revision: number; archived_at: string | null }>(
    'SELECT revision, archived_at FROM workspaces WHERE id = $1',
    [id]
  );
  assertArchiveTransition('workspace', id, revision, archived, rows[0]);
}
...
async function setWorkspaceArchived(
  ...
  if (!rows[0]) await assertWorkspaceArchiveTransition(id, revision, archived);
  return rows[0] ?? null;
}
