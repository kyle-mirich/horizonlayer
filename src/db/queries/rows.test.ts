...
describe('row persistence contracts', () => {
  ...
  it('enforces strict typed inputs and configured choices', async () => {
    ...
    await expect(createRow({ database_id: 'db-1', values: { Title: 'x', Due: '2026-02-30' } })).rejects.toThrow(
      'Due must be a valid date'
    );
  ...
  it('rejects invalid calendar dates in filters before querying rows', async () => {
    const dateProperties = [
      property(),
      property({ id: 'prop-due', name: 'Due', property_type: 'date', position: 1 }),
    ];
    mocks.poolQuery.mockImplementation(async (sql: string) => {
      if (sql.includes('FROM databases')) return { rows: [{ id: 'db-1', workspace_id: 'ws-1' }] };
      if (sql.includes('FROM database_properties')) return { rows: dateProperties };
      throw new Error(`Unexpected SQL: ${sql}`);
    });
    const { queryRows } = await import('./rows.js');
    ...
    await expect(queryRows({
      database_id: 'db-1',
      filters: [{ property: 'Due', operator: 'gt', value: '2026-02-30' }],
  ...
  it('rejects invalid calendar dates before mutating row updates', async () => {
    const dateProperties = [
      property(),
      property({ id: 'prop-due', name: 'Due', property_type: 'date', position: 1 }),
    ];
    ...
    expect(mocks.clientQuery.mock.calls.some(([sql]) =>
      String(sql).includes('UPDATE database_rows') || String(sql).includes('INSERT INTO database_row_values')
    )).toBe(false);
  });
