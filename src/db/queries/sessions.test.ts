...
describe('session query layer', () => {
  ...
  it('writes oversized resume bundles to /tmp instead of returning them inline', async () => {
    ...
    expect(result?.file_path).toMatch(/^\/tmp\/horizondb-session-session-1-\d+\.txt$/);
    expect(writeFileMock).toHaveBeenCalledWith(expect.stringMatching(/^\/tmp\/horizondb-session-session-1-\d+\.txt$/), expect.any(String), 'utf8');
