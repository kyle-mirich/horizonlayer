...
async function searchPages(params: {
  ...
  if (params.mode === 'full_text') {
    ...
    if (!isSystemAccess(params.access) && !isOrganizationAdmin(params.access)) { conditions.push('p.workspace_id IS NOT NULL'); values.push(requireUserAccess(params.access).userId); conditions.push(workspaceAccessPredicate('p.workspace_id', `$${idx++}`, 'read')); }
    ...
    const { rows } = await pool.query<{
      ...
      `SELECT p.id, p.title, p.workspace_id, p.tags,
              ts_rank(to_tsvector('english', p.title), plainto_tsquery('english', $${idx})) AS score,
              (SELECT b.content FROM blocks b
               WHERE b.page_id = p.id AND b.content != ''
               ORDER BY b.position LIMIT 1) AS snippet
       FROM pages p
       WHERE ${conditions.join(' AND ')}
         AND to_tsvector('english', p.title) @@ plainto_tsquery('english', $${idx})
       ORDER BY score DESC
       LIMIT ${params.limit}`,
      values
    );
  ...
  switch (params.mode) {
    ...
    case 'hybrid': {
      ...
      scoreExpr = `0.5 * (1 - (p.embedding <=> $${vecParam}::vector))
        ...
        + 0.1 * COALESCE(ts_rank(to_tsvector('english', p.title), plainto_tsquery('english', $${ftsParam})), 0)`;
      break;
    }
    default:
      scoreExpr = `1 - (p.embedding <=> $${vecParam}::vector)`;
...
async function searchRows(params: {
  ...
  if (params.mode === 'full_text') {
    return []; // rows lack a tsvector column
  }
  ...
  if (params.mode === 'grep') {
    ...
    if (!isSystemAccess(params.access) && !isOrganizationAdmin(params.access)) { conditions.push('d.workspace_id IS NOT NULL'); values.push(requireUserAccess(params.access).userId); conditions.push(workspaceAccessPredicate('d.workspace_id', `$${idx++}`, 'read')); }
    ...
    const { rows } = await pool.query<{
      ...
      `SELECT r.id, r.database_id, d.workspace_id, r.tags,
              ...
              (SELECT v.value_text
               ...
               LIMIT 1) AS title,
              (SELECT COALESCE(v2.value_text, v2.value_json::text, v2.value_number::text, v2.value_date::text)
