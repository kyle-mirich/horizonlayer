...
import { expandReference, isEntityReference } from '../../references.js';
...
function normalizeQuery(value: string): string {
  ...
  // A compact reference is a lossless UUID representation. Normalize it before
  // the SQL exact-ID branch so search behaves the same way as every ID field.
  return isEntityReference(query) ? expandReference(query) : query;
}
