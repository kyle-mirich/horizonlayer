  ...
  try {
    ...
    if (!touchResult.rows[0]) {
      if (expected_updated_at) {
        throw new Error(`Conflict: page ${pageId} was modified by another agent`);
      }
...
export async function listPages(params: {
  ...
  if (!isSystemAccess(access) && !isOrganizationAdmin(access)) {
    ...
    conditions.push(workspaceAccessPredicate('workspace_id', `$${idx++}`, 'read'));
  }
...
export async function updatePageBlock(
  ...
  const blockAccess = !isSystemAccess(access) ? await assertBlockWriteAccess(blockId, access) : null;
  const pageId = blockAccess?.page_id ?? (await getPool().query<{ page_id: string }>('SELECT page_id FROM blocks WHERE id = $1', [blockId])).rows[0]?.page_id;
