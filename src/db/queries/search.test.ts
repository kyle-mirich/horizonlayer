...
describe('canonical record search SQL', () => {
  ...
  it('normalizes lossless compact references for exact page and row matches', async () => {
    const pageId = 'c2b12324-eb92-4d55-a884-2b61d0b4fd30';
    const rowId = 'effb8d7a-41ae-45f1-bca5-8bd649535ef8';
    poolQueryMock
      .mockResolvedValueOnce({ rows: [pageRow({ id: pageId })] })
      .mockResolvedValueOnce({ rows: [rowResult({ id: rowId })] });

    const { searchRecords } = await import('./search.js');
    await searchRecords({
      query: compactReference('page', pageId),
      scope: {
        kind: 'workspace', workspace_id: 'ws-1', types: ['page'],
        session_id: null, database_id: null,
      },
      limit: 1,
    });
    await searchRecords({
      query: compactReference('row', rowId),
      scope: {
        kind: 'workspace', workspace_id: 'ws-1', types: ['row'],
        session_id: null, database_id: null,
      },
      limit: 1,
    });
