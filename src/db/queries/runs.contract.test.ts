...
describe('run query contracts', () => {
  ...
  it('checkpoints a running run transactionally and returns refreshed details', async () => {
    ...
    const lockedRunCallIndex = mocks.clientQuery.mock.calls.findIndex(([sql]) =>
      String(sql).includes('FROM agent_runs') && String(sql).includes('FOR UPDATE')
    );
    expect(mocks.lockActiveSessionForChildWrite.mock.invocationCallOrder[0]).toBeGreaterThan(
      mocks.clientQuery.mock.invocationCallOrder[lockedRunCallIndex]
    );
    expect(mocks.clientQuery.mock.calls.at(-1)?.[0]).toBe('COMMIT');
    expect(mocks.release).toHaveBeenCalledTimes(1);
  });

  it('rejects checkpointing a run after its attached session closes', async () => {
    mocks.clientQuery.mockImplementation(async (sql: string) => {
      if (sql === 'BEGIN' || sql === 'ROLLBACK') return { rowCount: 0, rows: [] };
      if (sql.includes('FROM agent_runs') && sql.includes('FOR UPDATE')) {
        return { rows: [buildRun()] };
    ...
    mocks.lockActiveSessionForChildWrite.mockRejectedValueOnce(
      new Error('Session session-1 is closed and cannot be modified')
    );

    const { checkpointRun } = await import('./runs.js');
    await expect(checkpointRun({ run_id: 'run-1', state: { cursor: 1 } })).rejects.toThrow(
      'Session session-1 is closed and cannot be modified'
    );
    ...
    expect(mocks.clientQuery.mock.calls.some(([sql]) =>
      String(sql).includes('INSERT INTO run_checkpoints')
    )).toBe(false);
    expect(mocks.clientQuery.mock.calls.at(-1)?.[0]).toBe('ROLLBACK');
    expect(mocks.release).toHaveBeenCalledTimes(1);
  });
