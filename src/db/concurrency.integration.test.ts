...
integrationDescribe('database concurrency invariants', () => {
  ...
  it('rejects checkpointing a session-bound run after its session closes', async () => {
    const setup = await adminPool.connect();
    ...
    await expect(closeSession(sessionId!)).resolves.toMatchObject({ status: 'closed' });
    await expect(checkpointRun({ run_id: run.id, state: { step: 1 } })).rejects.toThrow(
      `Session ${sessionId} is closed and cannot be modified`
    );

    const verifier = await adminPool.connect();
    try {
      await setTestSearchPath(verifier, schemaName);
      const { rows } = await verifier.query<{
        latest_checkpoint_sequence: number;
        checkpoint_count: number;
      }>(
        `SELECT r.latest_checkpoint_sequence,
                (SELECT COUNT(*)::int FROM run_checkpoints WHERE run_id = r.id) AS checkpoint_count
         FROM agent_runs r
         WHERE r.id = $1`,
        [run.id]
      );
      expect(rows[0]).toMatchObject({ latest_checkpoint_sequence: 0, checkpoint_count: 0 });
    } finally {
      verifier.release();
    }
  });
