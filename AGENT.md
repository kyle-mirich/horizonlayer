...
- `src/server.ts`: assembles the FastMCP server and registers the compact core tools.
...
- `examples/`: end-to-end MCP usage examples for memory and coordination.
...
3. `src/server.ts` creates the FastMCP server and registers `session`, `memory`, and `coordination`.
...
- "How is search implemented?" -> `src/tools/core.ts`, `src/db/queries/search.ts`, `src/embeddings/index.ts`
- "How do tasks and runs work?" -> `src/tools/core.ts`, `src/db/queries/tasks.ts`, `src/db/queries/runs.ts`
